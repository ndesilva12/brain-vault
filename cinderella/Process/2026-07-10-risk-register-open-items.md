# Cinderella Corp — Risk Register & Open Items
_Session: 2026-07-10 · living document — update status as items close_

Stress-test of the plan/model/deployment. Severity: 🔴 critical (could sink
it) · 🟠 major · 🟡 watch. Owner: N = Norman, S = Sunjay, L = legal/counsel.
...
| # | Gap | Why it matters | Owner |
|---|---|---|---|
| 12 | **NCAA NIL clearinghouse ("NIL Go" / Deloitte FMV review).** Post-House, large NIL deals get fair-market-value screened. | Needs dedicated collegiate-sports counsel, not just Loeb corporate. | L |
| 13 | **Institutional control / school approval process.** Board, president, AD, faculty buy-in — long and political. | School LOIs may take longer than the deployment timeline assumes. | N |
| 14 | **Tax structure.** SPV tax treatment, investor K-1s, UBIT if a nonprofit school participates, state taxes. | Unmodeled; affects net investor economics. | L |
...
| # | Gap | Why it matters | Owner |
|---|---|---|---|
| 15 | **Coach alignment.** A transformed roster with a misaligned/weak coach fails. Buy-in, retention, comp unaddressed. | Execution risk to on-court results, which the whole thesis depends on. | N |
| 16 | **Roster mechanics & timing.** Top-25 roster in one offseason = transfer-portal windows + tampering rules + agent execution. | Clock-driven and operationally hard; must be locked before the season. | N |
...
| # | Gap | Why it matters | Owner |
|---|---|---|---|
| 17 | **Bridge / working-capital timing.** Players paid up front (summer/fall); streaming + sponsor cash arrive later. | Timing gap needs a funding answer. | N/S |
| 18 | **Insurance beyond E&O/D&O** — key-person (Norman + celebrity), general liability, player-related. | Concentrated key-person risk uninsured. | S |
| 19 | **Downside case.** Model is all upside. No plan for a losing season, celebrity scandal, or no tournament run — yet the Cinderella thesis *requires* winning. | No stress/downside scenario in the model. | N |
...
| # | Gap | Why it matters | Owner |
|---|---|---|---|
| 20 | **Investor relations / finance function.** 15 investors need K-1s + reporting; SPV waterfalls need real accounting. Ops assistant won't cover it. | Grows into a real gap as SPVs launch. | S |
| 21 | **PR / communications & crisis comms.** Narrative management; a season can go badly (injury, scandal, losing). | No comms plan or owner. | N/S |
...
franchise rollup + promotion overlay into the live sheet._
