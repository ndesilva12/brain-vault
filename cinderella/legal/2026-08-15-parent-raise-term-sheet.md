...
_Prepared 2026-08-15 as an internal working draft for Norman to review with counsel (Loeb & Loeb — Brian Socolow) and Sunjay before any use. Nothing here is an offer to sell securities. Any offering must comply with federal/state securities law (Reg D). Bracketed items **[LIKE THIS]** need confirmation._

# Cinderella Corp LLC — Seed Round Summary of Principal Terms
...
| **Investor economics** | Per the one-pager (additive): a **temporary revenue share** — **50% of ManCo gross cash revenue collected** to Class A (pro rata) until **1.0×** ($1.5M), then **10%** until **2.0×** ($3.0M) cumulative, then it **terminates** — paid **in addition to** a **permanent 15% equity** (pro-rata distributions at all times, uncapped). |
...
| **Drag / Tag** | **Drag-along:** Manager may require Investors to participate in a Company sale on the same terms. **Tag-along:** Investors may participate pro rata if founders sell. **[Confirm — recommended so a sale isn't blocked.]** |
| **Securities compliance** | Private placement under **Reg D Rule 506(b)**; **accredited investors only**; Form D + state blue-sky notices to be filed by counsel; restricted units, standard investment-intent reps. |
| **Governing law / disputes** | Delaware law; **[venue / arbitration — confirm]**. |
| **Confidentiality / non-circumvention** | Investors bound by confidentiality + the existing Cinderella non-circumvention framework. |
| **Documents** | (1) this Term Sheet; (2) Subscription Agreement + Accredited Investor Questionnaire; (3) Amended & Restated Operating Agreement. |
| **Expenses** | Each party bears its own; Company pays its own formation/counsel costs. |
| **Conditions** | Execution of the A&R Operating Agreement and Subscription Agreement; satisfactory accreditation; **counsel (Loeb) sign-off on securities compliance.** |

---

## ⚠️ The open terms that actually matter (lock these with Loeb/Sunjay)

**1. Waterfall — DECIDED: additive, per the one-pager.** The revenue share (50%→1×, 10%→2×, then terminates) is a **temporary preferred return paid IN ADDITION TO the permanent 15% equity**, as the one-pager reads. Drafted this way in the OA (§4.1). **Consequence to action:** this pays Class A **more than 15% of total**, so the investor MOIC is **higher than the ~11×** in your model — **the financial model must be updated to match** (it currently computes a flat 15% and understates the investor take / overstates ManCo's share). Have counsel define the sweep basis vs. equity basis so the two streams don't unintentionally double-count.
